...
#define NUM_MBUF(ports, rx, tx, lcores)                                        \
  RTE_MAX((ports * rx * RX_DESC_DEFAULT + ports * lcores * MAX_PKT_BURST +     \
           ports * tx * TX_DESC_DEFAULT + lcores * MEMPOOL_CACHE_SIZE),        \
          (unsigned)8192)
...
static const struct rte_eth_conf port_conf = {
    .rxmode =
        {
            .mq_mode = RTE_ETH_MQ_RX_RSS, .max_lro_pkt_size = RTE_ETHER_MAX_LEN,
            //.split_hdr_size = 0,
            // .header_split   = 0,
            // .hw_ip_checksum = 1,
            // .hw_vlan_filter = 0,
...
static const struct option long_opts[] = {{CMD_LINE_OPT_NUMA_ON, 0, 0, 0},
...
static struct lcore_conf lcore_conf[MY_RTE_MAX_LCORE];
 ...
 */
static uint8_t socket_for_lcore(unsigned lcore_id) {
 ...
 */
static inline uint32_t csum_partial(const uint16_t *p, uint16_t size,
                                    uint32_t start) {
  uint32_t sum = start;
 ...
 */
static inline uint16_t ipv4_pseudo_csum(struct rte_ipv4_hdr *ip) {
  struct psd_hdr psd;
  ...
  psd.len = rte_cpu_to_be_16(rte_be_to_cpu_16(ip->total_length) -
                             (int)((ip->version_ihl & 0x0F) * 4u));
...
static void print_pkt(int src_ip, int dst_ip, uint16_t src_port,
  ...
  printf("rx: %u.%u.%u.%u:%u -> %u.%u.%u.%u:%u (%d bytes)\n", b[0], b[1], b[2],
         b[3], sp, b[6], b[7], b[8], b[9], dp, len);
}

/*
 * Send a number of packets out the specified port (Ethernet device).
 */
static inline int send_burst(struct lcore_conf *conf, uint16_t n, uint8_t port,
                             int socket) {
  uint16_t rv, queueid;
  ...
  // printf("rte_eth_tx_burst(port=%u, queue=%u, len=%u) -> %u\n", port,
  // queueid, n, rv);
  if (unlikely(rv < n)) {
    /* drop the packets we couldn't send */
    printf("drop the pkt\n");
    do {
      rte_pktmbuf_free(m_table[rv]);
    } while (++rv < n);
  }
  // RTE_LOG(DEBUG, USER1, "free count mempool socket %u: %u\n", socket,
  // rte_mempool_free_count(pktmbuf_pool[socket]));
  return 0;
}
 ...
 */
static inline int send_one(struct lcore_conf *conf, struct rte_mbuf *pkt,
                           uint8_t port, int socket) {
  uint16_t len;
...
static INLINE int echo_single_udp_packet(struct rte_mbuf *pkt) {
  ...
  l2_len = sizeof(*eth_h);
  /* frames in VPC come in with ethertype 0x8100, i.e. they are 802.1q VLAN
   * tagged */
  ...
  pkt->ol_flags |= (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_UDP_CKSUM);
  ip_h->hdr_checksum = 0;
  ip_h->hdr_checksum =
      rte_ipv4_cksum(ip_h); // need to pay attention to this, if set udp
                            // checksum, it will be filtered
...
static int main_loop() {
  int socket;
  uint8_t port, queue;
  uint16_t i, n_rx, n_reply;
  unsigned lcore_id;
  struct rte_mbuf *pkt, *pkts_burst[PKT_BURST];
  ...
  if (conf->n_rx_queue == 0) {
    printf("lcore %u not mapped into service\n", lcore_id);
    return 0;
  }
  printf("starting service on lcore %u\n", lcore_id);
  for (i = 0; i < conf->n_rx_queue; i++) {
    port = conf->rx_queues[i].port_id;
    queue = conf->rx_queues[i].queue_id;
    printf("-- lcore=%u port=%u rx_queue=%u\n", lcore_id, port, queue);
  }
  printf("conf->n_rx_queue:%d\n", conf->n_rx_queue);
  while (1) {
    for (i = 0; i < conf->n_rx_queue; i++) {
      port = conf->rx_queues[i].port_id;
      queue = conf->rx_queues[i].queue_id;
      n_reply = 0;
      printf("before receiving, port:%d, queue:%d, \n", port, queue);
      n_rx = rte_eth_rx_burst(port, queue, pkts_burst, PKT_BURST);
      if (unlikely(n_rx == 0)) {
         printf("no packet yet\n");
	 DIE("no packet yet\n");
        continue;
      }
      for (int j = 0; j < n_rx; j++) {
        ...
        // rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4) = 8
...
static void init_packet_buffers(unsigned num_mbuf) {
  ...
  for (lcore_id = 0; lcore_id < MY_RTE_MAX_LCORE; lcore_id++) {
    ...
    if (pktmbuf_pool[socketid] == NULL) {
      pktmbuf_pool[socketid] = rte_mempool_create(
          s, num_mbuf, MBUF_SIZE, MEMPOOL_CACHE_SIZE,
...
static void init_tx_queue_for_port(uint8_t port) {
  ...
  queue = 0;
  for (lcore_id = 1; lcore_id < MY_RTE_MAX_LCORE; lcore_id++) {
    if (!rte_lcore_is_enabled(lcore_id)) {
      continue;
    }
    socketid = socket_for_lcore(lcore_id);
    printf("initializing TX queue: (lcore %u, queue %u, socket %u)\n", lcore_id,
           queue, socketid);
    if ((rv =
             rte_eth_tx_queue_setup(port, queue, nb_txd, socketid, &tx_conf))) {
      rte_exit(EXIT_FAILURE,
               "rte_eth_tx_queue_setup(%u) failed: err=%d port=%d queue=%d\n",
               lcore_id, rv, port, queue);
    }
    printf("-- tx_queue: port=%d queue=%d socket=%d\n", port, queue, socketid);
...
static void check_lcore_params(void) {
  ...
  for (i = 0; i < n_lcore_params; i++) {
    ...
    if (socketid != 0 && !numa_on) {
      printf("lcore %hhu is on socket %d with NUMA off\n", lcore, socketid);
    }
  }
}
 ...
 */
static void configure_ports(unsigned n_ports, uint32_t port_mask,
                            uint32_t n_lcores) {
  int rv;
  uint8_t portid, n_rx_queue;
  uint32_t n_tx_queue;
  struct rte_ether_addr eth_addr;
  ...
  for (portid = 0; portid < n_ports; portid++) {
    ...
    // n_tx_queue = n_lcores;
    // tx and rx are set to 1
    n_rx_queue = 1;
    n_tx_queue = 1;
    if (n_tx_queue > MAX_TX_QUEUE_PER_PORT) {
      n_tx_queue = MAX_TX_QUEUE_PER_PORT;
    }
    printf("initializing port %u: %u rx, %u tx\n", portid, (uint16_t)n_rx_queue,
           n_tx_queue);
    if ((rv = rte_eth_dev_configure(portid, n_rx_queue, (uint16_t)n_tx_queue,
                                    &port_conf)) < 0) {
...
static void init_rx_queues(void) {
  int rv;
  uint8_t portid, queueid, socketid;
  uint16_t queue;
  unsigned lcore_id;
  struct lcore_conf *qconf;
  // change MY_RTE_MAX_LCORE to 1
  for (lcore_id = 0; lcore_id < 1; lcore_id++) {
    if (!rte_lcore_is_enabled(lcore_id)) {
      continue;
    }
    socketid = socket_for_lcore(lcore_id);
    ...
    for (queue = 0; queue < qconf->n_rx_queue; queue++) {
      ...
      printf("-- rx_queue: port=%u queue=%u socket=%u\n", portid, queueid,
             socketid);
      if ((rv = rte_eth_rx_queue_setup(portid, queueid, nb_rxd, socketid,
                                       &rx_conf, pktmbuf_pool[socketid]))) {
...
static void check_port_link_status(uint8_t n_ports, uint32_t port_mask) {
  uint8_t port, count, all_ports_up;
...
static int parse_rx_config(const char *arg) {
  enum fields { FLD_PORT = 0, FLD_QUEUE, FLD_LCORE, _NUM_FLD };

  int i;
  char s[256], *end, *str_fld[_NUM_FLD];
  unsigned size;
  const char *p, *p0 = arg;
...
#define LONG_EQ(label)                                                         \
  !strncmp(long_opts[optidx].name, (label), sizeof((label)))
  int rv, opt, optidx;
  char *prgname = argv[0];
  char **argvopt;

  argvopt = argv;
  while ((opt = getopt_long(argc, argvopt, "C:hNp:", long_opts, &optidx)) !=
         EOF) {
    switch (opt) {
    case 'C':
...
static void init_nics(void) {
  ...
  printf("%u Ethernet port(s) detected\n", n_ports);
